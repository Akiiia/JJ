...
def is_sequence_string(cards):
    #判断字符串是否连续
    card_values = {"3": 3, "4": 4, "5": 5, "6": 6, "7": 7, "8": 8, "9": 9, "T": 10, "J": 11, "Q": 12, "K": 13, "A": 14, "2": 15, "X": 16, "D": 17}
...
def check_cards(cards):
    ...
    if len(cards) >= 5 and len(set(cards)) == len(cards) and is_sequence_string(cards):
        return '单顺'

    if len(cards) >= 6 and len(set(cards)) == len(cards) / 2 and is_sequence_string(set(cards)) and max_char_count(cards) == 2:
        return '双顺'
